...
class PyramidSolver:
    ...
    def solve(self):
        ...
        while self.solver.check() == z3.sat:
            ...
            nb_sols += 1
            self.print_pyramid()
            print("\nSolution #%d took %2.3f second(s)" % (nb_sols, end_time - start_time))
        ...
        if nb_sols == 0:
            end_time = time.time()
            print("\nNo solutions found! Spent, %2.3f second(s)" % (end_time - start_time))
        else:
            print("Found %d total solution(s) and spent %2.3f second(s)" % (nb_sols, total_time))
