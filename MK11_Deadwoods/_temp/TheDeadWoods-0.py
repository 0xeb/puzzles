...
def find_path(graph, state, end_state, route = [], visited=set(), routes=[]):
    for button in BUTTONS.keys():
        ...
        new_state = graph.get(path, None)
        if new_state is None:
            return
        
        new_route = list(route)
        new_route.append('%s + %s -> %s' % (state, button, new_state))

        new_visited = set(visited)
        new_visited.add(path)

        if new_state == end_state:
            routes.append(new_route)
            if route:
                print("ROUTE[%d]: %s" % (len(new_route), ', '.join(new_route)))
            return True

        old_routes_count = len(routes)
        find_path(graph, new_state, end_state, new_route, new_visited, routes)
        if old_routes_count != len(routes):
            return True
...
routes = []
find_path(graph, 'SRH', 'RRR', routes=routes)
print(routes)